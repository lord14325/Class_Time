...
function Room() {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/api/rooms/${id}`, { method: 'DELETE' });
      const data = await response.json();

      if (!response.ok) {
        if (data.studentCount !== undefined || data.scheduleCount !== undefined) {
          let message = 'Cannot delete room:\n';
          if (data.studentCount > 0) {
            message += `\n• ${data.studentCount} student(s) assigned`;
          }
          if (data.scheduleCount > 0) {
            message += `\n• ${data.scheduleCount} schedule(s) assigned`;
          }
          alert(message);
        } else {
