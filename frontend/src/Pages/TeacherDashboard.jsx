...
function TeacherDashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (!userStr) {
          setError("Missing userId in localStorage.");
          return;
        }
        const userId = parseInt(userStr, 10);
        if (isNaN(userId)) {
          setError("Invalid userId in localStorage.");
          return;
        }
        ...
        if (!schedulesRes.ok) {
          const txt = await schedulesRes.text().catch(() => '');
          if (schedulesRes.status === 404) {
            setError("Teacher profile not found for current user.");
          } else {
            setError(`Failed to load schedule (${schedulesRes.status} ${txt})`);
          }
        } else {
          ...
          const mappedSchedules = (schedulesData || []).map(s => {
            const dayName = s.day_of_week && typeof s.day_of_week === 'number'
              ...
              : s.day_of_week || s.day || "Unknown";

            return {
              day: dayName,
              time: `${formatTime(s.start_time)} - ${formatTime(s.end_time)}`,
              courseName: s.course_name || s.courseName || "—",
              className: `${s.grade_level || ""} ${s.section_name || ""}`.trim(),
  ...
  return (
    <Layout>
      <div className="dashboard-welcome">
        <h1>Welcome, {teacherName}!</h1>
        <p className="subtitle">Your teaching schedule and announcements at a glance.</p>
      </div>

      {error && (
        <div className="error-banner" style={{ background: "#ffecec", color: "#900", padding: 12, borderRadius: 6, marginBottom: 12 }}>
          ...
          <ul className="schedule-list">
            {todaySchedule.map((s,i) => (
              <li key={i} className="schedule-item">
                ...
                <div className="class-info">{s.className} • {s.time} • Room {s.room}</div>
              </li>
            ))}
          </ul>
